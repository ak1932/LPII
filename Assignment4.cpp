...
//     string nibble_substitution(string state, vector<vector<char>> sbox, int len = 16) {
//         string result;
...
//         return state.substr(0, 4) + state.substr(12, 4) + state.substr(8, 4) + state.substr(4, 4);
//     }
...
class SAES{
    ...
    string shift_row(string word){
        return word.substr(0,4) + word.substr(12,4)+word.substr(8,4)+word.substr(4,4);
    }
    ...
    string inv_mix_col(string word){
        ...
        swap(n[1], n[2]);
        vector<int> result {
            gf_mul(9, n[0]) ^ gf_mul(2, n[2]),
            gf_mul(9, n[2]) ^ gf_mul(2, n[0]),
            gf_mul(9, n[1]) ^ gf_mul(2, n[3]),
            gf_mul(9, n[3]) ^ gf_mul(2, n[1])
        };
    ...
    string addRoundKey(string word, string key){
        return XOR(word, key);
    }
